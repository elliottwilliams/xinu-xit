...
#define assert_eq_fmt(lhs, rhs, fmt) do {                                                                  \
  if ((lhs) != (rhs)) {                                                                                    \
    _AS_FAIL;                                                                                              \
    _as_bprintf(                                                                                           \
        _AS_LPAD _AS_FAILURE_MESSAGE_PRE "assert_eq_fmt(" #lhs ", " #rhs ", %s)\n\n"                       \
        _AS_LPAD "Expected: " #lhs " == " fmt "\n"                                                         \
        _AS_LPAD "     Got: " fmt  " != " fmt "\n",                                                        \
        __FILE__, __LINE__, (fmt), (rhs), (lhs), (rhs));                                                   \
    return;                                                                                                \
  }                                                                                                        \
} while (0);

#define assert_neq_fmt(lhs, rhs, fmt) do {                                                                 \
  if ((lhs) == (rhs)) {                                                                                    \
    _AS_FAIL;                                                                                              \
    _as_bprintf(                                                                                           \
        _AS_LPAD _AS_FAILURE_MESSAGE_PRE "assert_neq_fmt(" #lhs ", " #rhs ", %s)\n\n"                      \
        _AS_LPAD "Expected: " #lhs " != " fmt "\n"                                                         \
        _AS_LPAD "     Got: " fmt  " == " fmt "\n",                                                        \
        __FILE__, __LINE__, (fmt), (rhs), (lhs), (rhs));                                                   \
    return;                                                                                                \
